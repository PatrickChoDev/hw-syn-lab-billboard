...
module keyboard
    ...
    input wire ps2_data, ps2_clk,               // ps2 data and clock lines
    ...
    output wire ctrl_key_active          // output signal for active Ctrl key
    );
    ...
    localparam  BREAK    = 8'hf0, // break code
                ...
                CTRL_LEFT = 8'h14, // left control key
                CTRL_RIGHT = 8'h11; // right control key
    ...
    localparam [2:0] lowercase          = 3'b000, // idle, process lower case letters
                     ...
                     control            = 3'b110, // process control key pressed
                     ignore_ctrl_break  = 3'b111;
    ...
    reg letter_case;                           // 0 for lower case, 1 for uppercase, outputted to use when converting scan code to ascii
    reg ctrl_key;
    ...
    reg [7:0] ctrl_type_reg, ctrl_type_next;           // register to track the control key status (pressed or not)

    // instantiate ps2 receiver
    ps2_rx ps2_rx_unit (.clk(clk), .reset(reset), .rx_en(1'b1), .ps2_data(ps2_data), .ps2_clk(ps2_clk), .rx_ready(scan_done_tick), .rx_data(scan_out));

    // FSM state, shift_type, caps_num, ctrl_key register
    ...
    always @*
        begin
            ...
            ctrl_type_next  = ctrl_type_reg;  // track the state of the Ctrl key
            ...
            lowercase:
                ...
                if(scan_done_tick)                                                                    // if scan code received
                    ...
                    else if(scan_out == CTRL_LEFT || scan_out == CTRL_RIGHT)                          // if code is either left or right control key
                        begin
                            ctrl_type_next = scan_out;                                                     // set ctrl_key to active (pressed)
                            state_next = control;                                                     // go to control state
                        end
            ...
            // state to ignore repeated scan code after break code F0 received in lowercase state
            ignore_break:
                begin
                if(scan_done_tick)                                                                    // if scan code received, 
            ...
            control:
                begin
                ctrl_key = 1'b1;                                                                   // routed out to convert scan code to control value for a key
                
                if(scan_done_tick)                                                                    // if scan code received,
                    begin
                    if(scan_out == BREAK)                                                             // if code is break code                                            
                        state_next = ignore_ctrl_break;                                              // go to ignore_ctrl_break state to ignore repeated scan code after F0
    
                    else if(scan_out != CTRL_LEFT && scan_out != CTRL_RIGHT)                        // else if code is not control key
                        got_code_tick = 1'b1;                                                         // assert got_code_tick to write scan_out to FIFO
                    end
                end
            ...
            ignore_ctrl_break:
                begin
                 if(scan_done_tick)                                                                // if scan code received
                     begin
                     if(scan_out == ctrl_type_reg)                                                // if scan code is shift key initially pressed
                         state_next = lowercase;                                                   // shift/capslock key unpressed, go back to lowercase state
                     else                                                                          // else repeated scan code received, go back to uppercase state
                         state_next = control;
                     end
    ...
    // output, route got_code_tick to outer control circuit to signal when to sample scan_out 
    assign scan_code_ready = got_code_tick;
    
    // route scan code data out
    assign scan_code = scan_out;

    // output the control key active signal
    assign ctrl_key_active = ctrl_key;
